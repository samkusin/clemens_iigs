...
static void _clem_rtc_dispatch_cmd(struct ClemensDeviceRTC *rtc, unsigned data) {
...
void _clem_rtc_bram_state(struct ClemensDeviceRTC *rtc, unsigned data) {
...
uint8_t _clem_rtc_bram_read(struct ClemensDeviceRTC *rtc) {
...
void _clem_rtc_bram_write(struct ClemensDeviceRTC *rtc, unsigned data) {
    /*
...
uint8_t _clem_rtc_clock_read(struct ClemensDeviceRTC *rtc) {
    ...
    if (opt & 1) {
        if (rtc->index & 0x80000000) {
            ...
            if (opt & 0x4)
                return (uint8_t)((rtc->seconds_since_1904 >> 24) & 0xff);
            else
                return (uint8_t)((rtc->seconds_since_1904 >> 16) & 0xff);
        } else {
            /* lo portion of clock data */
            if (opt & 0x4)
                return (uint8_t)((rtc->seconds_since_1904 >> 8) & 0xff);
            else
                return (uint8_t)(rtc->seconds_since_1904 & 0xff);
        }
    }
...
void clem_rtc_command(struct ClemensDeviceRTC *rtc, clem_clocks_time_t ts, unsigned op) {
    ...
    */
    ...
    if (op == CLEM_IO_WRITE) {
        ...
        if (rtc->xfer_started_time + rtc->xfer_latency_duration > ts)
            return;
