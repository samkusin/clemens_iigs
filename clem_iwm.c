...
static uint8_t s_lss_rom[256] = {
    };

static inline unsigned
_clem_iwm_get_access_state(struct ClemensDeviceIWM *iwm) {
...
static bool _clem_iwm_lss_write_async(struct ClemensDeviceIWM *iwm,
                                      struct ClemensClock *clock,
...
static void _clem_iwm_debug_null(struct ClemensDeviceIWM *iwm,
                                 struct ClemensClock *clock) {}

static void _clem_iwm_debug(struct ClemensDeviceIWM *iwm,
                            struct ClemensDrive *drive,
                            struct ClemensClock *clock) {
  if (iwm->io_flags & CLEM_IWM_FLAG_READ_DATA_FAKE) {
    CLEM_LOG("iwm: @[track=%u, base=%u, offset=%u:%u] fake_bit = %u",
             drive->real_track_index,
             drive->disk.track_byte_offset[drive->real_track_index],
             drive->track_byte_index, drive->track_bit_shift,
             (iwm->io_flags & CLEM_IWM_FLAG_READ_DATA) ? 1 : 0);
  }
}
...
static void _clem_iwm_lss(struct ClemensDeviceIWM *iwm,
  ...
  */
  ...
  if (iwm->io_flags & CLEM_IWM_FLAG_DRIVE_35) {
    ...
    if (iwm->enable_debug) {
      _clem_iwm_debug_null(iwm, clock);
    }
    return;
  }
    ...
    if (cmd & 0x08) {
      ...
      case 0x0F:
        ...
        if (iwm->state & CLEM_IWM_STATE_WRITE_MASK) {
          ...
          CLEM_WARN("IWM: state: %02X load byte %02X in read?", iwm->state,
                    iwm->data);
        }
        break;
...
void clem_iwm_glu_sync(struct ClemensDeviceIWM *iwm,
  ...
  if (iwm->io_flags & CLEM_IWM_FLAG_DRIVE_ON) {
    ...
    if (iwm->ns_drive_hold > 0) {
      iwm->ns_drive_hold =
          clem_util_timer_decrement(iwm->ns_drive_hold, delta_ns);
...
void _clem_iwm_io_switch(struct ClemensDeviceIWM *iwm,
                         struct ClemensDriveBay *drives,
...
static uint8_t _clem_iwm_read_handshake(struct ClemensDeviceIWM *iwm,
  ...
  unsigned ns_write_overflow = sync_fast_mode
                                   ? (8 * 2 * 4 * iwm->lss_update_dt_ns)
