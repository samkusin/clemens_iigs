...
struct ClemensSerializerRecord kADBMouse[] = {
    CLEM_SERIALIZER_RECORD_ARRAY(struct ClemensDeviceMouse, kClemensSerializerTypeUInt32, pos,
                                 CLEM_ADB_KEYB_BUFFER_LIMIT, 0),
    CLEM_SERIALIZER_RECORD_INT16(struct ClemensDeviceMouse, mx),
    CLEM_SERIALIZER_RECORD_INT16(struct ClemensDeviceMouse, my),
    CLEM_SERIALIZER_RECORD_INT16(struct ClemensDeviceMouse, mx0),
    CLEM_SERIALIZER_RECORD_INT16(struct ClemensDeviceMouse, my0),
    ...
    CLEM_SERIALIZER_RECORD_BOOL(struct ClemensDeviceMouse, tracking_enabled),
    CLEM_SERIALIZER_RECORD_BOOL(struct ClemensDeviceMouse, valid_clamp_box),
    CLEM_SERIALIZER_RECORD_EMPTY()};
...
struct ClemensSerializerRecord kEnsoniq[] = {
    ...
    CLEM_SERIALIZER_RECORD_UINT8(struct ClemensDeviceEnsoniq, data_reg),
...
struct ClemensSerializerRecord kSCCChannel[] = {
    ...
    CLEM_SERIALIZER_RECORD_ARRAY(struct ClemensDeviceSCCChannel, kClemensSerializerTypeUInt8, regs,
                                 16, 0),
    CLEM_SERIALIZER_RECORD_UINT8(struct ClemensDeviceSCCChannel, rr0),
    ...
    CLEM_SERIALIZER_RECORD_DURATION(struct ClemensDeviceSCCChannel, master_clock_step),
    ...
    CLEM_SERIALIZER_RECORD_UINT32(struct ClemensDeviceSCCChannel, rx_shift_ctr),
    CLEM_SERIALIZER_RECORD_UINT32(struct ClemensDeviceSCCChannel, rx_register),
    CLEM_SERIALIZER_RECORD_UINT32(struct ClemensDeviceSCCChannel, brg_counter),
...
static void clemens_unserialize_records(mpack_reader_t *reader, uintptr_t data_adr,
    ...
    char key[64];
    while (record->type != kClemensSerializerTypeEmpty) {
        mpack_expect_cstr_match(reader, record->name);
...
static unsigned clemens_unserialize_custom(mpack_reader_t *reader, void *ptr, unsigned sz,
    ...
    case CLEM_SERIALIZER_CUSTOM_RECORD_AUDIO_MIX_BUFFER:
        ...
        mpack_expect_cstr_match(reader, "frame_count");
        audio_mix_buffer->frame_count = mpack_expect_u32(reader);
        mpack_expect_cstr_match(reader, "frames_per_second");
        audio_mix_buffer->frames_per_second = mpack_expect_u32(reader);
        mpack_expect_cstr_match(reader, "stride");
    ...
    case CLEM_SERIALIZER_CUSTOM_RECORD_NIBBLE_DISK:
        ...
        clemens_unserialize_records(reader, (uintptr_t)nib_disk, &kNibbleDisk[0], alloc_cb,
                                    context);
        mpack_expect_cstr_match(reader, "bits_data");
        if (mpack_expect_bool(reader)) {
            mpack_expect_cstr_match(reader, "blob");
...
mpack_reader_t *clemens_unserialize_machine(mpack_reader_t *reader, ClemensMachine *machine,
    ...
    mpack_expect_cstr_match(reader, "object");
    ...
    */
    mpack_expect_cstr_match(reader, "banks");
    ...
    mpack_expect_cstr_match(reader, "mega2");
    mpack_expect_array(reader);
