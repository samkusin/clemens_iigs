...
SOKOL_GFX_API_DECL void sg_query_image_pixels(sg_image img_id, void *pixels,
                                              int size);
SOKOL_GFX_API_DECL void sg_query_pixels(int x, int y, int w, int h,
                                        bool origin_top_left, void *pixels,
                                        int size);
SOKOL_GFX_API_DECL void sg_update_texture_filter(sg_image img_id,
...
static void _sg_all_convert_to_rgba(void *dest_pixels, int dest_w, int dest_h,
  ...
  unsigned char *in_pixels = (unsigned char *)src_pixels;
  int row, col, src_bytes_per_pixel = src_a >= 0 ? 4 : 3;
  int dest_offset;
  for (row = 0, dest_offset = 0; row < dest_h; row++) {
    unsigned char *out_pixels = (unsigned char *)dest_pixels + row * dest_w * 4;
    unsigned char *in_pixels = (unsigned char *)src_pixels + row * src_pitch;
    for (col = 0; col < dest_w; col++) {
      out_pixels[0] = in_pixels[src_r];
      out_pixels[1] = in_pixels[src_g];
      out_pixels[2] = in_pixels[src_b];
      out_pixels[3] = src_a >= 0 ? in_pixels[src_a] : 0xff;
      out_pixels += 4;
      in_pixels += src_bytes_per_pixel;
    }
  }
}
...
static void _sg_gl_query_image_pixels(_sg_image_t *img, void *pixels) {
  ...
  _sg_gl_cache_bind_texture(0, img->gl.target,
  ...
  glReadPixels(0, 0, img->cmn.width, img->cmn.height, GL_RGBA, GL_UNSIGNED_BYTE,
               pixels);
...
static void _sg_gl_query_pixels(int x, int y, int w, int h,
                                bool origin_top_left, void *pixels) {
  SOKOL_ASSERT(pixels);
  GLuint gl_fb;
  GLint dims[4];
...
static void _sg_gl_update_texture_filter(_sg_image_t *img, sg_filter min_filter,
                                         sg_filter mag_filter) {
  _sg_gl_cache_store_texture_binding(0);
  _sg_gl_cache_bind_texture(0, img->gl.target,
                            img->gl.tex[img->cmn.active_slot]);
...
static inline void _sgext_d3d11_Texture2D_GetDesc(ID3D11Texture2D *self,
...
static inline void
_sgext_d3d11_SamplerState_GetDesc(ID3D11SamplerState *self,
...
static inline void _sgext_d3d11_CopySubresourceRegion(
...
#if defined(__cplusplus)
  self->CopySubresourceRegion(pDstResource, DstSubresource, DstX, DstY, DstZ,
                              pSrcResource, SrcSubresource, pSrcBox);
#else
  self->lpVtbl->CopySubresourceRegion(self, pDstResource, DstSubresource, DstX,
...
static inline void
_sgext_d3d11_OMGetRenderTargets(ID3D11DeviceContext *self, UINT NumViews,
                                ID3D11RenderTargetView **ppRenderTargetViews,
                                ID3D11DepthStencilView **ppDepthStencilView) {
#if defined(__cplusplus)
  self->OMGetRenderTargets(NumViews, ppRenderTargetViews, ppDepthStencilView);
#else
  self->lpVtbl->OMGetRenderTargets(self, NumViews, ppRenderTargetViews,
                                   ppDepthStencilView);
#endif
}

static inline void
_sgext_d3d11_RenderTargetView_GetResource(ID3D11RenderTargetView *self,
...
static void _sg_d3d11_query_image_pixels(_sg_image_t *img, void *pixels) {
  ...
  hr = _sg_d3d11_CreateTexture2D(_sg.d3d11.dev, &staging_desc, NULL,
                                 &staging_tex);
  SOKOL_ASSERT(SUCCEEDED(hr));

  // copy pixels to staging texture
  _sgext_d3d11_CopySubresourceRegion(
      _sg.d3d11.ctx, (ID3D11Resource *)staging_tex, 0, 0, 0, 0,
      (ID3D11Resource *)img->d3d11.tex2d, 0, NULL);

  // map the staging texture's data to CPU-accessible memory
  D3D11_MAPPED_SUBRESOURCE msr = {};
  msr.pData = NULL;
  hr = _sg_d3d11_Map(_sg.d3d11.ctx, (ID3D11Resource *)staging_tex, 0,
                     D3D11_MAP_READ, 0, &msr);
  SOKOL_ASSERT(SUCCEEDED(hr));

  // copy the data into the desired buffer, converting pixels to the desired
  // format at the same time
  switch (staging_desc.Format) {
  case DXGI_FORMAT_B8G8R8A8_UNORM:
    _sg_all_convert_to_rgba(pixels, img->cmn.width, img->cmn.height, msr.pData,
                            msr.RowPitch, 2, 1, 0, 3);
    break;
  case DXGI_FORMAT_B8G8R8X8_UNORM:
    _sg_all_convert_to_rgba(pixels, img->cmn.width, img->cmn.height, msr.pData,
                            msr.RowPitch, 2, 1, 0, -1);
    break;
  default:
    break;
  }
  ...
  if (staging_tex)
    _sg_d3d11_Release(staging_tex);
}

static void _sg_d3d11_query_pixels(int x, int y, int w, int h,
                                   bool origin_top_left, void *pixels) {
  ...
  _sgext_d3d11_RenderTargetView_GetResource(render_target_view,
  ...
  staging_desc.Usage = D3D11_USAGE_STAGING;
  ID3D11Texture2D *staging_tex = NULL;
  HRESULT hr = _sg_d3d11_CreateTexture2D(_sg.d3d11.dev, &staging_desc, NULL,
                                         &staging_tex);
  SOKOL_ASSERT(SUCCEEDED(hr));
  _SOKOL_UNUSED(hr);
  ...
  _sgext_d3d11_CopySubresourceRegion(
      _sg.d3d11.ctx, (ID3D11Resource *)staging_tex, 0, 0, 0, 0,
      (ID3D11Resource *)back_buffer, 0, &src_box);
  ...
  switch (staging_desc.Format) {
  case DXGI_FORMAT_B8G8R8A8_UNORM:
    _sg_all_convert_to_rgba(pixels, w, h, msr.pData, msr.RowPitch, 2, 1, 0, 3);
    break;
  case DXGI_FORMAT_B8G8R8X8_UNORM:
    _sg_all_convert_to_rgba(pixels, w, h, msr.pData, msr.RowPitch, 2, 1, 0, -1);
    break;
  default:
    break;
  }
  ...
  if (back_buffer)
    _sg_d3d11_Release(back_buffer);
  if (staging_tex)
    _sg_d3d11_Release(staging_tex);
}

static void _sg_d3d11_update_texture_filter(_sg_image_t *img,
                                            sg_filter min_filter,
                                            sg_filter mag_filter) {
  ...
  hr = _sg_d3d11_CreateSamplerState(_sg.d3d11.dev, &d3d11_smp_desc,
                                    &img->d3d11.smp);
...
static void _sg_metal_commit_command_buffer() {
  ...
  if (_sg.mtl.cmd_buffer) {
    ...
    [_sg.mtl.uniform_buffers[_sg.mtl.cur_frame_rotate_index]
...
static void _sg_metal_encode_texture_pixels(int x, int y, int w, int h,
                                            bool origin_top_left,
                                            id<MTLTexture> mtl_src_texture,
                                            void *pixels) {
  SOKOL_ASSERT(!_sg.mtl.in_pass);
  _sg_metal_commit_command_buffer();
  MTLTextureDescriptor *mtl_dst_texture_desc = [MTLTextureDescriptor
      texture2DDescriptorWithPixelFormat:mtl_src_texture.pixelFormat
                                   width:w
                                  height:h
                               mipmapped:NO];
  mtl_dst_texture_desc.storageMode = MTLStorageModeManaged;
  mtl_dst_texture_desc.resourceOptions = MTLResourceStorageModeManaged;
  mtl_dst_texture_desc.usage =
      MTLTextureUsageShaderRead + MTLTextureUsageShaderWrite;
  id<MTLTexture> mtl_dst_texture =
      [mtl_src_texture.device newTextureWithDescriptor:mtl_dst_texture_desc];
  ...
  [mtl_dst_texture getBytes:temp_pixels
                bytesPerRow:w * 4
                 fromRegion:mtl_region
                mipmapLevel:0];
  switch (mtl_dst_texture_desc.pixelFormat) {
  case MTLPixelFormatBGRA8Unorm:
    _sg_all_convert_to_rgba(pixels, w, h, temp_pixels, w * 4, 2, 1, 0, 3);
  case MTLPixelFormatRGBA8Unorm:
    _sg_all_convert_to_rgba(pixels, w, h, temp_pixels, w * 4, 0, 1, 2, 3);
  default:
    break;
  }
...
static void _sg_metal_query_image_pixels(_sg_image_t *img, void *pixels) {
  ...
  _sg_metal_encode_texture_pixels(0, 0, mtl_src_texture.width,
                                  mtl_src_texture.height, true, mtl_src_texture,
                                  pixels);
}

static void _sg_metal_query_pixels(int x, int y, int w, int h,
                                   bool origin_top_left, void *pixels) {
  id<CAMetalDrawable> mtl_drawable =
      (__bridge id<CAMetalDrawable>)_sg.mtl.drawable_cb();
  _sg_metal_encode_texture_pixels(x, y, w, h, origin_top_left,
                                  mtl_drawable.texture, pixels);
}

static void _sg_metal_update_texture_filter(_sg_image_t *img,
                                            sg_filter min_filter,
                                            sg_filter mag_filter) {
