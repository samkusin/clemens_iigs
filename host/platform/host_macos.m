...
- (BOOL)queryInputsAtIndex: (NSUInteger)atIndex withJoystick:(ClemensHostJoystick *)inputs;
...
- (BOOL)queryInputsAtIndex: (NSUInteger)index withJoystick:(ClemensHostJoystick *)inputs {
    if (index >= controllers.count) return NO;
    ...
    GCExtendedGamepad* gamepad = [controller extendedGamepad];
    GCMicroGamepad* micro = [controller microGamepad];
    if (gamepad == nil && micro == nil) return NO;

    inputs->isConnected = true;
    inputs->buttons = 0;
    if (gamepad) {
        inputs->x[0] = gamepad.leftThumbstick.xAxis.value * CLEM_HOST_JOYSTICK_AXIS_DELTA;
        inputs->x[1] = gamepad.rightThumbstick.xAxis.value * CLEM_HOST_JOYSTICK_AXIS_DELTA;
        inputs->y[0] = -gamepad.leftThumbstick.yAxis.value * CLEM_HOST_JOYSTICK_AXIS_DELTA;
        inputs->y[1] = -gamepad.rightThumbstick.yAxis.value * CLEM_HOST_JOYSTICK_AXIS_DELTA;;
...
unsigned clem_joystick_poll(ClemensHostJoystick *joysticks) {
    ...
    if (controllerDispatch_) {
        for (; index < controllerDispatch_.controllerCount && index < CLEM_HOST_JOYSTICK_LIMIT; ++index) {
            if ([controllerDispatch_ queryInputsAtIndex: index withJoystick: &joysticks[index]] == NO) {
