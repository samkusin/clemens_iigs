...
static unsigned s_baud_rates[] = {300, 1200, 2400, 4800, 9600, 
                                  19200, 38400, 57600, 0};
...
void clem_serial_peer_set_baud_rate(ClemensSerialPeer *peer, enum ClemensSerialBaudRate baud_rate) {
    ...
    peer->baud_gen_clocks_dt = (s_xtal_frequency / s_baud_rates[baud_rate]) *
        CLEM_CLOCKS_28MHZ_CYCLE;        
}
...
void clem_serial_peer_transact(ClemensSerialPeer *peer, struct ClemensClock *clock,
    ...
    while (dt >= peer->baud_gen_clocks_dt) {
