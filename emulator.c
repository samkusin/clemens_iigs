  ...
  do {                                                                         \
    ...
    if (_cpu_->pins.emulation) {                                               \
      printf(ANSI_COLOR_GREEN                                                  \
             "Clocks.... Cycles...." ANSI_COLOR_RESET                          \
             ...
             _cpu_->regs.D, (_cpu_->regs.A & 0xff00) >> 8,                     \
             _cpu_->regs.A & 0x00ff, _cpu_->regs.X & 0x00ff,                   \
             _cpu_->regs.Y & 0x00ff);                                          \
...
static inline void
_opcode_instruction_define_mvn(struct ClemensInstruction *instr, uint8_t opcode,
...
static inline void _opcode_instruction_define(struct ClemensInstruction *instr,
                                              uint8_t opcode, uint16_t value,
                                              bool opc_8) {
  instr->desc = &sOpcodeDescriptions[opcode];
  instr->bank = 0x00;
  instr->opc_8 = opc_8;
...
static inline void
_opcode_instruction_define_long(struct ClemensInstruction *instr,
                                uint8_t opcode, uint8_t bank, uint16_t addr) {
  instr->desc = &sOpcodeDescriptions[opcode];
  instr->bank = bank;
  instr->opc_8 = false;
  instr->value = addr;
...
static inline void
_opcode_instruction_define_dp(struct ClemensInstruction *instr, uint8_t opcode,
                              uint8_t offset) {
  instr->desc = &sOpcodeDescriptions[opcode];
  instr->bank = 0x00;
  instr->opc_8 = false;
...
static void _opcode_print(ClemensMachine *clem,
  ...
  case kClemensCPUAddrMode_PCRelative:
    snprintf(operand, sizeof(operand), "$%02X (%d)", inst->value,
             (int8_t)inst->value);
    break;
  case kClemensCPUAddrMode_PCRelativeLong:
    snprintf(operand, sizeof(operand), "$%04X (%d)", inst->value,
             (int16_t)inst->value);
    break;
  ...
  case kClemensCPUAddrMode_MoveBlock:
    snprintf(operand, sizeof(operand), "s:%02X, d:%02X", inst->value & 0xff,
             inst->bank);
    break;
  }
  ...
  if (clem->debug_flags & kClemensDebugFlag_DebugLogOpcode) {
    ...
    int debug_len =
        snprintf(debug_text, 32, "%2u %02X:%04X %s %s", inst->cycles_spent,
                 inst->pbr, inst->addr, inst->desc->name, operand);
    memset(debug_text + debug_len, 0x20, 32 - debug_len);
    debug_text[31] = '\n';
  }
  if ((clem->debug_flags & kClemensDebugFlag_OpcodeCallback) &&
      clem->opcode_post) {
    (*clem->opcode_post)(inst, operand, clem->debug_user_ptr);
  }
}

void _clem_debug_memory_dump(ClemensMachine *clem, uint8_t mem_page,
                             uint8_t mem_bank, uint8_t page_count) {
  char filename[64];
  ...
  if (fp) {
    ...
    while (page_count > 0) {
      ...
      bool mega2 = false;
      fwrite(_clem_get_memory_bank(clem, mem_bank, &mega2) + mem_addr, 256, 1,
             fp);
...
void _clem_init_instruction_map() {
  ...
  _opcode_description(CLEM_OPC_STA_ABSL, "STA",
                      kClemensCPUAddrMode_AbsoluteLong);
...
int clemens_init(ClemensMachine *machine, uint32_t speed_factor,
                 uint32_t clocks_step, void *rom, size_t romSize, void *e0bank,
                 void *e1bank, void *fpiRAM, void *slotExpansionROM,
  ...
  for (idx = 0xfc; idx <= 0xff; ++idx) {
    ...
    machine->fpi_bank_map[idx] =
        (uint8_t *)rom + CLEM_IIGS_BANK_SIZE * (idx - 0xfc);
  }
...
void clemens_simple_init(ClemensMachine *machine, uint32_t speed_factor,
                         uint32_t clocks_step, void *fpiRAM,
                         unsigned int fpiRAMBankCount) {
...
bool clemens_load_hex(ClemensMachine *clem, const char *hex,
                      const char *hex_end, unsigned bank) {
  const char *line = hex;
...
bool clemens_assign_disk(ClemensMachine *clem, enum ClemensDriveType drive_type,
  ...
  /* filter out 'bad' disk/drive pairings before the IWM has a chance to flag
     them
  */
  if (drive_type == kClemensDrive_5_25_D1 ||
      drive_type == kClemensDrive_5_25_D2) {
    if (disk->disk_type != CLEM_DISK_TYPE_5_25) {
      return false;
    }
  } else if (drive_type == kClemensDrive_3_5_D1 ||
             drive_type == kClemensDrive_3_5_D2) {
...
void clemens_input(ClemensMachine *machine,
...
void cpu_execute(struct Clemens65C816 *cpu, ClemensMachine *clem) {
  ...
  */
  ...
  case CLEM_OPC_ADC_DP_IDX:
    _clem_read_pba_mode_dp(clem, &tmp_addr, &tmp_pc, &tmp_data, cpu->regs.X,
                           x_status);
  ...
  case CLEM_OPC_BMI:
    _clem_read_pba(clem, &tmp_data, &tmp_pc);
    _clem_branch(clem, &tmp_pc, tmp_data,
  ...
  case CLEM_OPC_LDX_DP_IDY:
    _clem_read_pba_mode_dp(clem, &tmp_addr, &tmp_pc, &tmp_data, cpu->regs.Y,
                           x_status);
    _clem_read_data_816(clem, &tmp_value, tmp_addr, 0x00, x_status);
    _cpu_ldxy(cpu, &cpu->regs.X, tmp_value, x_status);
    _opcode_instruction_define_dp(&opc_inst, IR, tmp_data);
  ...
  case CLEM_OPC_PHD:
    ...
    clem_write(clem, (uint8_t)(cpu->regs.D >> 8), cpu->regs.S, 0x00,
               CLEM_MEM_FLAG_DATA);
    clem_write(clem, (uint8_t)(cpu->regs.D), cpu->regs.S - 1, 0x00,
               CLEM_MEM_FLAG_DATA);
    _cpu_sp_dec2(cpu);
    break;
  case CLEM_OPC_PHK:
  ...
  case CLEM_OPC_STA_ABSL_IDX:
    _clem_read_pba_mode_absl(clem, &tmp_addr, &tmp_bnk0, &tmp_pc);
    _clem_write_indexed_816(clem, cpu->regs.A, tmp_addr, cpu->regs.X, tmp_bnk0,
                            m_status, x_status);
    _opcode_instruction_define_long(&opc_inst, IR, tmp_bnk0, tmp_addr);
    break;
  ...
  case CLEM_OPC_STA_DP_INDIRECTL_IDY:
    _clem_read_pba_mode_dp_indirectl(clem, &tmp_addr, &tmp_bnk0, &tmp_pc,
                                     &tmp_data, 0, false);
    _clem_write_indexed_816(clem, cpu->regs.A, tmp_addr, cpu->regs.Y, tmp_bnk0,
                            m_status, x_status);
    _opcode_instruction_define_dp(&opc_inst, IR, tmp_data);
    break;
  ...
  case CLEM_OPC_STZ_ABS_IDX:
    _clem_read_pba_mode_abs(clem, &tmp_addr, &tmp_pc);
    _clem_cycle(clem, 1);
    _clem_write_indexed_816(clem, 0x0000, tmp_addr, cpu->regs.X, cpu->regs.DBR,
                            m_status, x_status);
    _opcode_instruction_define(&opc_inst, IR, tmp_addr, m_status);
    break;
...
void clemens_emulate(ClemensMachine *clem) {
  ...
  if (!cpu->pins.resbIn) {
    ...
    */
    if (cpu->state_type != kClemensCPUStateType_Reset) {
      ...
      cpu->regs.P |= (kClemensCPUStatus_MemoryAccumulator |
                      kClemensCPUStatus_Index | kClemensCPUStatus_IRQDisable);
  ...
  if (cpu->state_type == kClemensCPUStateType_Reset) {
    ...
    cpu->regs.PC = _clem_read_interrupt_vector(
        clem, CLEM_6502_RESET_VECTOR_LO_ADDR, CLEM_6502_RESET_VECTOR_HI_ADDR);
    if (!clem->mmio_bypass) {
  ...
  if (!clem->mmio_bypass) {
    ...
    //      like VGC
    delta_mega2_cycles =
        (uint32_t)((clem->clocks_spent / clem->clocks_step_mega2) -
                   mmio->mega2_cycles);
