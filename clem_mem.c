...
static inline void _clem_mem_cycle(ClemensMachine *clem, bool mega2_access) {
  clem->clocks_spent +=
      mega2_access ? clem->clocks_step_mega2 : (clem->clocks_step);
  ++clem->cpu.cycles_spent;
}

static void
_clem_mmio_create_page_direct_mapping(struct ClemensMemoryPageInfo *page,
                                      uint8_t page_idx) {
  page->read = page_idx;
  page->write = page_idx;
...
static void
_clem_mmio_create_page_mainaux_mapping(struct ClemensMemoryPageInfo *page,
...
void clem_mem_create_page_mapping(struct ClemensMemoryPageInfo *page,
                                  uint8_t page_idx, uint8_t bank_read_idx,
...
static inline void _clem_mmio_newvideo_c029_set(struct ClemensMMIO *mmio,
                                                uint8_t value) {
...
static void _clem_mmio_slotrom_select_c02d(struct ClemensMMIO *mmio,
                                           uint8_t data) {
  int i;
  unsigned slot_mask = CLEM_MEM_IO_MMAP_CROM &
                       (~(CLEM_MEM_IO_MMAP_CXROM | CLEM_MEM_IO_MMAP_C3ROM));
  ...
  if (mmio->diagnostic) {
    printf("SKS: @%02X:%04X C02D: %02x -> %x\n", mmio->diagnostic->PBR,
           mmio->diagnostic->PC, data, mmap_register);
  }
#endif
...
static void _clem_mmio_shadow_c035_set(struct ClemensMMIO *mmio,
                                       uint8_t value) {
...
static void _clem_mmio_speed_c036_set(ClemensMachine *clem, uint8_t value) {
...
static void _clem_mmio_vgc_irq_c023_set(struct ClemensMMIO *mmio,
                                        uint8_t data) {
  if (data & 0x4) {
...
static uint8_t _clem_mmio_statereg_c068_set(struct ClemensMMIO *mmio,
                                            uint8_t value) {
...
static uint8_t _clem_mmio_rw_bank_select(struct ClemensMMIO *mmio,
                                         uint16_t address) {
...
static uint8_t _clem_mmio_card_io_read(ClemensCard *card,
                                       struct ClemensClock *clock, uint8_t addr,
                                       uint8_t flags) {
  uint8_t result = 0;
  if (card) {
...
static uint8_t _clem_mmio_read(ClemensMachine *clem, uint16_t addr,
  ...
  if (flags & CLEM_OP_IO_CARD) {
    ...
    if (addr == 0xCFFF) {
      ...
      if (clem->card_slot[slot_idx]) {
        result = _clem_mmio_card_io_read(clem->card_slot[slot_idx], &ref_clock,
  ...
  case CLEM_MMIO_REG_VGC_TEXT_COLOR:
    result =
        (uint8_t)((mmio->vgc.text_fg_color << 4) | mmio->vgc.text_bg_color);
    break;
  ...
  case CLEM_MMIO_REG_DISK_INTERFACE:
    result = clem_iwm_read_switch(&mmio->dev_iwm, &clem->active_drives,
                                  &ref_clock, ioreg, flags);
    break;
  ...
  case CLEM_MMIO_REG_TXTPAGE1:
    if (!(flags & CLEM_OP_IO_NO_OP)) {
      _clem_mmio_memory_map(mmio,
                            mmio->mmap_register & ~CLEM_MEM_IO_MMAP_TXTPAGE2);
    }
    break;
  ...
  default:
    if (ioreg >= 0x71 && ioreg < 0x80) {
      ...
      result = _clem_mmio_card_io_read(clem->card_slot[(ioreg - 0x90) >> 4],
                                       &ref_clock, ioreg & 0xf, flags);
    } else if (!is_noop) {
...
static void _clem_mmio_card_io_write(ClemensCard *card,
                                     struct ClemensClock *clock, uint8_t data,
...
static void _clem_mmio_write(ClemensMachine *clem, uint8_t data, uint16_t addr,
                             uint8_t flags, bool *mega2_access) {
  struct ClemensMMIO *mmio = &clem->mmio;
  struct ClemensClock ref_clock;
  bool is_noop = (flags & CLEM_OP_IO_NO_OP) != 0;
  ...
  *mega2_access = true;
  ...
  if ((flags & CLEM_OP_IO_CARD) && addr >= 0xC100) {
    ...
    if (clem->card_slot[slot_idx]) {
      _clem_mmio_card_io_write(clem->card_slot[slot_idx], &ref_clock, data,
                               ioreg, flags | CLEM_OP_IO_DEVSEL);
    }
    return;
  }
  ...
  case CLEM_MMIO_REG_STDZP:
    _clem_mmio_memory_map(mmio,
                          mmio->mmap_register & ~CLEM_MEM_IO_MMAP_ALTZPLC);
    break;
  ...
  case CLEM_MMIO_REG_SPKR:
    ...
    break;
  case CLEM_MMIO_REG_DISK_INTERFACE:
    clem_iwm_write_switch(&mmio->dev_iwm, &clem->active_drives, &ref_clock,
                          ioreg, data);
    break;
  case CLEM_MMIO_REG_RTC_VGC_SCANINT:
    if (!(data & 0x40)) {
  ...
  default:
    if (ioreg >= 0x80) {
      _clem_mmio_card_io_write(clem->card_slot[(ioreg - 0x90) >> 4], &ref_clock,
                               data, ioreg & 0xf, flags);
    }
    if (!is_noop) {
      clem_debug_break(&mmio->dev_debug, &clem->cpu,
...
static void _clem_mmio_shadow_map(struct ClemensMMIO *mmio,
  ...
  */
  ...
  bool inhibit_shgr_bank_01 =
      (shadow_flags & CLEM_MEM_IO_MMAP_NSHADOW_SHGR) != 0;
...
static void _clem_mmio_memory_map(struct ClemensMMIO *mmio,
  ...
  if ((remap_flags & CLEM_MEM_IO_MMAP_CXROM) && mmio->diagnostic) {
    printf("SKS: @%02X:%04X remap %0x occuring to %0x\n", mmio->diagnostic->PBR,
           mmio->diagnostic->PC, remap_flags, memory_flags);
  }
#endif
  ...
  if (remap_flags & CLEM_MEM_IO_MMAP_OLDVIDEO) {
    for (page_idx = 0x04; page_idx < 0x08; ++page_idx) {
      ...
      if (memory_flags & CLEM_MEM_IO_MMAP_80COLSTORE) {
        page_B00->bank_read =
            ((memory_flags & CLEM_MEM_IO_MMAP_TXTPAGE2) ? 0x01 : 00);
        page_B00->bank_write =
            ((memory_flags & CLEM_MEM_IO_MMAP_TXTPAGE2) ? 0x01 : 00);
      } else {
        page_B00->bank_read =
        ...
        page_B00->bank_write =
            ((memory_flags & CLEM_MEM_IO_MMAP_RAMWRT) ? 0x01 : 00);
      }
    }
    for (page_idx = 0x20; page_idx < 0x40; ++page_idx) {
      page_B00 = &page_map_B00->pages[page_idx];
      if (memory_flags &
          (CLEM_MEM_IO_MMAP_80COLSTORE + CLEM_MEM_IO_MMAP_HIRES)) {
        page_B00->bank_read =
            ((memory_flags & CLEM_MEM_IO_MMAP_TXTPAGE2) ? 0x01 : 00);
        page_B00->bank_write =
            ((memory_flags & CLEM_MEM_IO_MMAP_TXTPAGE2) ? 0x01 : 00);
  ...
  if (remap_flags & CLEM_MEM_IO_MMAP_LC) {
    bool is_rom_bank_0x = !(memory_flags & CLEM_MEM_IO_MMAP_NIOLC) &&
                          !(memory_flags & CLEM_MEM_IO_MMAP_RDLCRAM);
...
void _clem_mmio_init_page_maps(struct ClemensMMIO *mmio,
  ...
  for (page_idx = 0x00; page_idx < 0x100; ++page_idx) {
    _clem_mmio_create_page_mainaux_mapping(&page_map->pages[page_idx], page_idx,
                                           0x00);
  }
  ...
  for (bank_idx = 0x02; bank_idx < CLEM_IIGS_FPI_MAIN_RAM_BANK_COUNT;
       ++bank_idx) {
    mmio->bank_page_map[bank_idx] = &mmio->fpi_direct_page_map;
  }
  /* TODO: handle expansion RAM */
  for (bank_idx = CLEM_IIGS_FPI_MAIN_RAM_BANK_COUNT; bank_idx < 0x80;
       ++bank_idx) {
...
void _clem_mmio_init(struct ClemensMMIO *mmio,
  ...
  _clem_mmio_init_page_maps(mmio, CLEM_MEM_IO_MMAP_NSHADOW_SHGR |
...
void clem_read(ClemensMachine *clem, uint8_t *data, uint16_t adr, uint8_t bank,
  ...
  if (page->flags & CLEM_MEM_IO_MEMORY_MASK) {
    ...
    if (page->flags & CLEM_MEM_PAGE_IOADDR_FLAG) {
      *data = _clem_mmio_read(clem, offset, read_only ? CLEM_OP_IO_NO_OP : 0,
                              &mega2_access);
    } else if (page->flags & CLEM_MEM_PAGE_CARDMEM_FLAG) {
      *data = _clem_mmio_read(
          clem, ((uint16_t)page->read << 8) | (adr & 0xff),
...
void clem_write(ClemensMachine *clem, uint8_t data, uint16_t adr, uint8_t bank,
                uint8_t mem_flags) {
  struct ClemensMemoryPageMap *bank_page_map = clem->bank_page_map[bank];
  ...
  if (page->flags & CLEM_MEM_IO_MEMORY_MASK) {
    ...
    if (page->flags & CLEM_MEM_PAGE_IOADDR_FLAG) {
      if (page->flags & CLEM_MEM_PAGE_WRITEOK_FLAG) {
        _clem_mmio_write(clem, data, offset, flags, &mega2_access);
      } else {
        mega2_access = true;
      }
    } else if (page->flags & CLEM_MEM_PAGE_CARDMEM_FLAG) {
      _clem_mmio_write(clem, data, ((uint16_t)page->write << 8) | (adr & 0xff),
    ...
    if (shadow_map && shadow_map->pages[page->write]) {
      bank_mem = _clem_get_memory_bank(clem, (0xE0) | (bank_actual & 0x1),
                                       &mega2_access);
      if (page->flags & CLEM_MEM_PAGE_WRITEOK_FLAG) {
        bank_mem[offset] = data;
      }
